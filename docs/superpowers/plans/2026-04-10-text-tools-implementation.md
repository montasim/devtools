...
**Tech Stack:** Next.js 15 (App Router), TypeScript, React hooks, diff library, react-diff-viewer-continued, shadcn/ui components, localStorage for persistence
...
- [ ] **Step 1: Add diff library dependencies**

Run: `pnpm add diff@^5.1.0 react-diff-viewer-continued@^3.2.6`
...
git commit -m "feat(text-tools): install diff and react-diff-viewer dependencies"
```
...
## Task 6: Create Diff Results Viewer
    ...
    const styles: ReactDiffViewerStylesOverride = {
        variables: {
            dark: {
                ...
                addedGutterColor: 'hsl(var(--diff-added-text, 144 70% 25%))',
                removedGutterColor: 'hsl(var(--diff-removed-text, 0 70% 25%))',
                codeFoldContentColor: 'hsl(var(--muted-foreground))',
                diffViewerTitleLineHeight: '1.5',
                diffViewerContentLineHeight: '1.6',
                diffViewerCodeFontSize: '13px',
            },
        },
        line: {
            padding: '4px 8px',
...
## Task 7: Create Diff Pane Component
...
import { exceedsSizeLimit, formatSize, MAX_TEXT_SIZE } from '../shared/types';
...
export const TextDiffPane = ({
    ...
    // Save left text to localStorage (debounced effect handled by parent)
    useEffect(() => {
        if (typeof window === 'undefined') return;
        try {
            localStorage.setItem('text-diff-left-content', leftText);
        } catch (error) {
            console.error('Failed to save left text to localStorage:', error);
            onError?.(new Error('Warning: Unable to save to browser storage (quota exceeded). Your work is not being saved.'));
        }
    }, [leftText, onError]);

    // Save right text to localStorage
    useEffect(() => {
        if (typeof window === 'undefined') return;
        try {
            localStorage.setItem('text-diff-right-content', rightText);
        } catch (error) {
            console.error('Failed to save right text to localStorage:', error);
            onError?.(new Error('Warning: Unable to save to browser storage (quota exceeded). Your work is not being saved.'));
        }
    }, [rightText, onError]);
    ...
    // Expose methods via ref (not used in current implementation but good for extensibility)
    useEffect(() => {
        const ref: TextDiffPaneRef = {
            triggerCompare: async () => {
                await computeDiff();
            },
            clear: handleClear,
        };
        // Could be exposed via forwardRef if needed
    }, [computeDiff, handleClear]);
...
git commit -m "feat(text-tools): add diff pane component with localStorage persistence"
```

---

## Task 8: Create Transform Pane Types
...
## Task 9: Create Transformation Utility Functions
...
## Task 10: Create Transform Logic Hook
...
## Task 11: Create Transform Actions Toolbar
...
## Task 12: Create Transform Pane Component
...
export const TextTransformPane = ({
    ...
    useEffect(() => {
        if (typeof window === 'undefined') return;
        try {
            localStorage.setItem('text-transform-input', inputText);
        } catch (error) {
            console.error('Failed to save input text to localStorage:', error);
            onError?.(new Error('Warning: Unable to save to browser storage (quota exceeded). Your work is not being saved.'));
        }
    }, [inputText, onError]);
...
git commit -m "feat(text-tools): add transform pane component with localStorage persistence"
```

---

## Task 13: Create Main Text Page with Tabs
...
- [ ] Toggle "Ignore Case" and compare texts with different cases
- [ ] Toggle "Ignore Whitespace" and compare texts with different spacing
...
- [ ] Click "Clear All" and verify both panes are cleared
- [ ] Test invalid Base64 decode and verify error message appears
- [ ] Test with text > 1MB and verify warning message appears
...
## Task 15: Add Diff Viewer Theme CSS Variables (Optional Polish)

**Files:**

- Modify: `app/globals.css` or create new CSS module
